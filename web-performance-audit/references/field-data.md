...
PageSpeed Insights can run without an API key. Direct CrUX and CrUX History API queries require a Google Cloud API key enabled for the Chrome UX Report API. When a key is available, the secret contract is one exact environment variable: `CRUX_API_KEY`. The host, operator, or secret manager must inject it into the process that launches the agent. The skill does not discover secret values from frontmatter, the filesystem, or the user’s local machine.

Use the bundled client for API calls. It reads `CRUX_API_KEY` internally when present, so the agent’s command contains neither the variable name as an argument nor its expanded value. It also redacts the key if Google unexpectedly echoes it in a successful or failed response.

Do not enumerate environment variables, search `.env` files, inspect keychains, or ask the user to paste the key into chat. If a direct CrUX request reports that `CRUX_API_KEY` is unavailable:
...
The command exits successfully only when `CRUX_API_KEY` is non-empty. It never prints the value. This check is not a prerequisite for anonymous PSI.
...
PSI runs anonymously when `CRUX_API_KEY` is absent and automatically uses the key when it is present. Anonymous PSI is suitable for occasional use; if Google rejects or throttles it, configure the key and retry once.
